namespace IFMIS.Report.Design.Payroll
{
    ...
    /// </summary>
    public partial class rpt_PEI : Telerik.Reporting.Report
    {
        public rpt_PEI(int year, int month, int qtr, int payreffid, int reportID, int employmentstatusid, int officeid, int payrefftypeid)
        {
            //
            ...
            txt_batchno.Value = "Batch No.: " + ISfn.ExecScalar("select [BonusBatchno] from dbo.[tbl_Payroll_Bonus] where payreffID = 42 and employmentstatus_id = " + employmentstatusid + " and year_ = " + year + " and officeID = " + officeid + " and PayreffTypeID = " + payrefftypeid + " order by BonusBatchno desc").ToString();
            textBox2.Value = "Productivity Enhancement Incentive(" + emptype + ")";
